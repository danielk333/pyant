...
def e3d_subarray(freqeuncy):
    ...
    x0_p1 = np.arange(-2.5, -5.5, -.5).tolist()
    x0_p2 = np.arange(-4.5, -2.0, .5).tolist()
...
def e3d_array(freqeuncy, fname=None, configuration='full'):
    ...
    if fname is None:
        dat = _read_e3d_submodule_pos(pkg_resources.resource_string(
            'pyant.beams.data', 'e3d_subgroup_positions.txt'))
    else:
