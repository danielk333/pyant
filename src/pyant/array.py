...
def plane_wave(k, r, p, J):
    ...
    :param numpy.ndarray p: Beam-forming direction by phase offsets 
...
class Array(Beam):
    ...
    def __init__(self, 
        ...
        if len(antennas.shape) == 2:
            antennas = antennas.reshape(
                antennas.shape[0], antennas.shape[1], 1)
    ...
    def gain(
        ...
        if vectorized_parameters:
            raise NotImplementedError(
                'vectorized_parameters is not supported by Array')
...
class DipoleArray(Array):
    ...
    def antenna_element(self, k, polarization):
        ...
        ret[0, ...] = k[2, :] * \
            (np.sin(az - antr)*polarization[0] +
             np.cos(az - antr)*polarization[1])
        ret[1, ...] = 0.0
